...
describe("Budget schema consistency", () => {
  it("lighthouse-budgets.json is valid JSON with required fields", () => {
    const data = JSON.parse(readFileSync(join(import.meta.dirname ?? ".", "..", "budgets", "lighthouse-budgets.json"), "utf-8"));
  ...
  it("cwv-budgets.json has all 9 templates", () => {
    const data = JSON.parse(readFileSync(join(import.meta.dirname ?? ".", "..", "budgets", "cwv-budgets.json"), "utf-8"));
    assert.ok(Object.keys(data.templates).length >= 9, "9 templates required");
  });
  it("k6-thresholds.json has valid threshold expressions", () => {
    const data = JSON.parse(readFileSync(join(import.meta.dirname ?? ".", "..", "budgets", "k6-thresholds.json"), "utf-8"));
