...
records the reason, the compensating control and the revised date as an ADR in
...
| **QA and testing**         | This repository (`viavitae-qa`), its CI pipelines, synthetic fixtures and all staging environments it targets.                                                                     |
...
and CI fails on any _verified_ finding; (2) pre-commit hooks run the same
scanner locally; (3) any credential suspected of exposure is rotated first,
history purge second — a leaked _live_ secret is a rotation event regardless
